				...
				*/
				for (int j=0; j<n_Sobs+n_Dobs; j++) {
					...
					for (int i=0; i<N+1; i++) {
						...
						AD<double> c = CppAD::pow(dx, 2) + CppAD::pow(dy, 2) - CppAD::pow(rad+var,2) - COL_THRESH;
						fg[pf] = c;
						AD<double> one(10.f);
						AD<double> zero(0.f);
						fg[0] += CppAD::CondExpLt(c, one, CppAD::exp(-CppAD::log(c)), zero);
